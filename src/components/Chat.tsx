...
const Chat = ({ onNavigateToListen }: { onNavigateToListen?: () => void }) => {
  ...
  const uploadFile = async (file: Blob, ext: string): Promise<string | null> => {
    ...
    const { data: signedData } = await supabase.storage.from('chat-media').createSignedUrl(fileName, 3153600000);
            ...
            return (
              <div key={msg.id} className="flex justify-center py-1">
                <div className="bg-muted/60 text-muted-foreground text-xs rounded-full px-4 py-1.5 flex items-center gap-1.5">
                  ...
                  {isMissed && currentUser === msg.sender && (
                    <span className="ml-1 text-[10px] text-muted-foreground/70 italic">You called</span>
                  )}
